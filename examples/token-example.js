...
const { createToken, verifyToken, getPublicKey } = require("./token");
// Shorthand utility binding
const print = console.log;

/*  Req object mimicking req objects before being processed by the createToken method.
...
function promise_version () {
  ...
  createToken(req.token, { subject: "bryan@gmail.com" })
    .then(token => {
      print(token);
      print(token.length);
      return token;
...
async function asyncawait_version () {
  ...
  try {
    const token = await createToken(req.token);
    print(token);
    print(token.length);
